  ...
  const confirmImageSelection = () => {
    ...
    try {
      ...
      const chatMessage: SupabaseChatMessage = {
          content: `[Фото ${tempSelectedImage.match(/\/(\d+)\.jpg$/)?.[1] || ''}] [${preloadedImages.find(img => img.url === tempSelectedImage)?.prompt || 'Пользователь отправил изображение'}]${priceInfo}${commentInfo} [чаттер отправил фото]`,
      ...
      let messagesToSend = chatHistory.map(msg => ({
        ...
        content: msg.imageUrl 
            ? `[Фото ${msg.imageUrl.match(/\/(\d+)\.jpg$/)?.[1] || ''}] [${preloadedImages.find(img => img.url === msg.imageUrl)?.prompt || 'Пользователь отправил изображение'}]${msg.price ? ` [Цена: ${msg.price}]` : ''} [чаттер отправил фото]` 
          : msg.content
