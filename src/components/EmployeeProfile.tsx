...
function EmployeeProfile() {
  ...
  useEffect(() => {
    ...
    const fetchEmployeeData = async () => {
      try {
        ...
        // Получаем полный профиль сотрудника (включая форму кандидата) через API
        ...
        // Устанавливаем данные формы кандидата
        if (data.candidate_form) {
          console.log('Получены данные формы кандидата:', data.candidate_form);
          setCandidateForm(data.candidate_form);
        } else {
          console.log('Форма кандидата отсутствует');
  ...
  const Tabs = () => {
    const tabs = [
      { id: 'profile', name: 'Анкета', icon: <User className="w-4 h-4" /> },
